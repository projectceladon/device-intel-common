...
ifeq ($(BUILD_REL10_BOOTCORE), true)
bootcore:
	$(MAKE) -C $(CURDIR)/../modem/system-build/make PLATFORM=$(MODEM_PLATFORM) PROJECTNAME=$(MODEM_PROJECTNAME_VAR) MAKEDIR=$(BOOTLOADER_BIN_PATH) $(MODEM_BUILD_ARGUMENTS) INT_STAGE=BOOTSYSTEM ADD_FEATURE+=FEAT_BOOTSYSTEM_PROXY_INFO_TO_MOBILEVISOR ADD_FEATURE+=FEAT_BOOTSYSTEM_DRV_MOBILEVISOR bootsystem
else
bootcore: $(BUILT_LIBSOC_TARGET)
	$(MAKE) -C $(CURDIR)/bootsystems/make PLATFORM=$(MODEM_PLATFORM) PROJECTNAME=$(MODEM_PROJECTNAME_VAR) BL_OUTPUT_DIR=$(BL_OUTPUT_DIR) SOC_LIB=$(BUILT_LIBSOC_TARGET)
endif
...
ifneq ($(INTEL_DDR_CTL_PARAMS_FILE),)
  ifeq ($(BUILD_REL10_BOOTCORE), true)
$(PSI_RAM_FLS): createflashloader_dir $(FLSTOOL) $(BOARD_PRG_FILE) $(BUILT_PSI_RAM_HEX) $(BUILT_PSI_RAM_XOR) $(SIGN_TOOL) $(PSI_RAM_DDRINJECT_SCRIPT) $(BOOTLDR_TMP_DIR)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $(PSI_RAM_BEFORE_DDR_INJECT_FLS) --script $(BUILT_PSI_RAM_XOR) --tag PSI_RAM $(BUILT_PSI_RAM_HEX) --replace --to-fls2
	echo Injecting custom DDR parameters to $@
	$(SIGN_TOOL) $(PSI_RAM_BEFORE_DDR_INJECT_FLS) -s $(PSI_RAM_DDRINJECT_SCRIPT) -o $@
else
$(PSI_RAM_FLS): createflashloader_dir $(FLSTOOL) $(BOARD_PRG_FILE) $(BUILT_PSI_RAM_HEX) $(BUILT_PSI_RAM_XOR) $(BUILT_PSI_RAM_VER) $(SIGN_TOOL) $(PSI_RAM_DDRINJECT_SCRIPT) $(BOOTLDR_TMP_DIR)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $(PSI_RAM_BEFORE_DDR_INJECT_FLS) --script $(BUILT_PSI_RAM_XOR) --meta $(BUILT_PSI_RAM_VER) --tag PSI_RAM $(BUILT_PSI_RAM_HEX) --replace --to-fls2
	echo Injecting custom DDR parameters to $@
	$(SIGN_TOOL) $(PSI_RAM_BEFORE_DDR_INJECT_FLS) -s $(PSI_RAM_DDRINJECT_SCRIPT) -o $@
endif # REL10_BOOTCORE
else
   ifeq ($(BUILD_REL10_BOOTCORE), true)
$(PSI_RAM_FLS): createflashloader_dir $(BOARD_PRG_FILE) $(FLSTOOL) $(BUILT_PSI_RAM_XOR) $(BUILT_PSI_RAM_HEX)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $@ --script $(BUILT_PSI_RAM_XOR) --tag PSI_RAM $(BUILT_PSI_RAM_HEX) --replace --to-fls2
$(EBL_FLS): createflashloader_dir $(BOARD_PRG_FILE) $(FLSTOOL) $(BUILT_EBL_HEX)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $@ --tag EBL $(BUILT_EBL_HEX) --replace --to-fls2
else
$(PSI_RAM_FLS): createflashloader_dir $(BOARD_PRG_FILE) $(FLSTOOL) $(BUILT_PSI_RAM_XOR) $(BUILT_PSI_RAM_HEX) $(BUILT_PSI_RAM_VER)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $@ --script $(BUILT_PSI_RAM_XOR) --meta $(BUILT_PSI_RAM_VER) --tag PSI_RAM $(BUILT_PSI_RAM_HEX) --replace --to-fls2
$(EBL_FLS): createflashloader_dir $(BOARD_PRG_FILE) $(FLSTOOL) $(BUILT_EBL_HEX) $(BUILT_EBL_VER) $(PSI_RAM_FLS)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $@ --meta $(BUILT_EBL_VER) --tag EBL $(BUILT_EBL_HEX) --replace --to-fls2 --psi $(PSI_RAM_FLS)
endif # REL10_BOOTCORE
endif
...
ifneq ($(INTEL_DDR_CTL_PARAMS_FILE),)
  ifeq ($(BUILD_REL10_BOOTCORE), true)
$(PSI_FLASH_FLS): createflashfile_dir $(FLSTOOL) $(BOARD_PRG_FILE) $(BUILT_PSI_FLASH_HEX) $(BUILT_PSI_FLASH_XOR) $(FLASHLOADER_FLS) $(SIGN_TOOL) $(PSI_FLASH_DDRINJECT_SCRIPT) $(BOOTLDR_TMP_DIR)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $(PSI_FLASH_BEFORE_DDR_INJECT_FLS) --script $(BUILT_PSI_FLASH_XOR) --tag PSI_FLASH $(BUILT_PSI_FLASH_HEX) $(INJECT_FLASHLOADER_FLS) --replace --to-fls2
	echo Injecting custom DDR parameters to $@
	$(SIGN_TOOL) $(PSI_FLASH_BEFORE_DDR_INJECT_FLS) --noinjremove -s $(PSI_FLASH_DDRINJECT_SCRIPT) -o $@
else
$(PSI_FLASH_FLS): createflashfile_dir $(FLSTOOL) $(BOARD_PRG_FILE) $(BUILT_PSI_FLASH_HEX) $(BUILT_PSI_FLASH_XOR) $(BUILT_PSI_FLASH_VER) $(FLASHLOADER_FLS) $(SIGN_TOOL) $(PSI_FLASH_DDRINJECT_SCRIPT) $(BOOTLDR_TMP_DIR)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $(PSI_FLASH_BEFORE_DDR_INJECT_FLS) --script $(BUILT_PSI_FLASH_XOR) --meta $(BUILT_PSI_FLASH_VER) --tag PSI_FLASH $(BUILT_PSI_FLASH_HEX) $(INJECT_FLASHLOADER_FLS) --replace --to-fls2
	echo Injecting custom DDR parameters to $@
	$(SIGN_TOOL) $(PSI_FLASH_BEFORE_DDR_INJECT_FLS) --noinjremove -s $(PSI_FLASH_DDRINJECT_SCRIPT) -o $@
endif # REL10_BOOTCORE
else
   ifeq ($(BUILD_REL10_BOOTCORE), true)
$(PSI_FLASH_FLS): createflashfile_dir $(BOARD_PRG_FILE) $(FLSTOOL) $(BUILT_PSI_FLASH_HEX) $(BUILT_PSI_FLASH_XOR) $(FLASHLOADER_FLS)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $@ --script $(BUILT_PSI_FLASH_XOR) --tag PSI_FLASH $(BUILT_PSI_FLASH_HEX) $(INJECT_FLASHLOADER_FLS) --replace --to-fls2

$(SLB_FLS): createflashfile_dir $(BOARD_PRG_FILE) $(FLSTOOL) $(BUILT_SLB_HEX) $(FLASHLOADER_FLS)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $@ --tag SLB $(BUILT_SLB_HEX) $(INJECT_FLASHLOADER_FLS) --replace --to-fls2
else
$(PSI_FLASH_FLS): createflashfile_dir $(BOARD_PRG_FILE) $(FLSTOOL) $(BUILT_PSI_FLASH_HEX) $(BUILT_PSI_FLASH_XOR) $(BUILT_PSI_FLASH_VER) $(FLASHLOADER_FLS)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $@ --script $(BUILT_PSI_FLASH_XOR) --meta $(BUILT_PSI_FLASH_VER) --tag PSI_FLASH $(BUILT_PSI_FLASH_HEX) $(INJECT_FLASHLOADER_FLS) --replace --to-fls2

$(SLB_FLS): createflashfile_dir $(BOARD_PRG_FILE) $(FLSTOOL) $(BUILT_SLB_HEX) $(BUILT_SLB_VER) $(FLASHLOADER_FLS)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $@ --meta $(BUILT_SLB_VER) --tag SLB $(BUILT_SLB_HEX) $(INJECT_FLASHLOADER_FLS) --replace --to-fls2
endif # REL10_BOOTCORE
endif # DDR_CTRL
...
UCODE_PATCH_BIN        := $(CURDIR)/device/intel/sofia3g/$(TARGET_PRODUCT)/ucode_patch/ucode_patch.bin
...
ifeq ($(BUILD_REL10_BOOTCORE), true)
$(UCODE_PATCH_FLS): createflashfile_dir $(BOARD_PRG_FILE) $(FLSTOOL) $(UCODE_PATCH_BIN) $(FLASHLOADER_FLS)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $@ --tag UCODE_PATCH $(UCODE_PATCH_BIN) $(INJECT_FLASHLOADER_FLS) --replace --to-fls2
else
$(UCODE_PATCH_FLS): createflashfile_dir $(BOARD_PRG_FILE) $(FLSTOOL) $(UCODE_PATCH_BIN) $(FLASHLOADER_FLS)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $@ --tag UC_PATCH $(UCODE_PATCH_BIN) $(INJECT_FLASHLOADER_FLS) --replace --to-fls2
endif
endif
...
SPLASH_IMG_FILE_1      		:= $(CURDIR)/device/intel/sofia3g/$(TARGET_PRODUCT)/splash_image/splash_screen.jpg
SPLASH_IMG_FILE_2      		:= $(CURDIR)/device/intel/sofia3g/$(TARGET_PRODUCT)/splash_image/fastboot.jpg
...
DTC							:= device/intel/sofia3g-kernel/i386/dtc
...
$(SPLASH_IMG_FLS): createflashfile_dir $(BOARD_PRG_FILE) $(FLSTOOL) $(SPLASH_IMG_BIN_0) $(SPLASH_IMG_BIN_1) $(SPLASH_IMG_BIN_2) $(FLASHLOADER_FLS)
	echo "$(FLSTOOL)"
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $@ --tag SPLASH_SCRN $(SPLASH_IMG_BIN_0) $(SPLASH_IMG_BIN_1) $(SPLASH_IMG_BIN_2) $(INJECT_FLASHLOADER_FLS) --replace --to-fls2
...
BOOTROM_PATCH_BIN      := $(CURDIR)/device/intel/sofia3g/$(TARGET_PRODUCT)/bootrom_patch/bootrom_patch.bin
BOOTROM_PATCH_FLS      := $(FLASHFILES_DIR)/bootrom_patch.fls

$(BOOTROM_PATCH_FLS): createflashfile_dir $(BOARD_PRG_FILE) $(FLSTOOL) $(BOOTROM_PATCH_BIN) $(FLASHLOADER_FLS)
	$(FLSTOOL) --prg $(BOARD_PRG_FILE) --output $@ --tag BM_PATCH $(BOOTROM_PATCH_BIN) $(INJECT_FLASHLOADER_FLS) --replace --to-fls2
