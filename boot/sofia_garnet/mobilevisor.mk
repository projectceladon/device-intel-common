...
ifeq ($(GEN_MV_RAM_DEFS_FROM_XML), true)
$(MV_RAM_DEFS_DEST_FILE): force | $(VMM_BUILD_OUT)
	mkdir -p $(VMM_BUILD_OUT)/mobilevisor
	if [ -a $(MV_RAM_DEFS_FILE) ]; \
	then cp -f $(MV_RAM_DEFS_FILE) $(MV_RAM_DEFS_DEST_FILE); \
...
build_vmm_target: $(BUILT_LIBSOC_TARGET) $(BUILT_LIB_MOBILEVISOR_SVC_TARGET)
	...
	make -C $(MOBILEVISOR_SRC_PATH) PROJECTNAME=$(shell echo $(TARGET_BOARD_PLATFORM_VAR) | tr a-z A-Z) BASEBUILDDIR=$(VMM_BUILD_OUT) WHOLE_ARCHIVE_LIB_LIST+="$(BUILT_LIBSOC_TARGET) $(BUILT_LIB_MOBILEVISOR_SVC_TARGET) $(abspath $(BUILT_MV_CORE_BIN))" PLATFORM=$(MODEM_PLATFORM) C_DEFINES=$(MOBILEVISOR_OPTION_common) INCLUDEDIR=$(MOBILEVISOR_INCLUDE_common)
...
build_vmm_target: $(BUILT_LIBSOC_TARGET)
	@echo Building ===== mobilevisor ======
	make -C $(MOBILEVISOR_SRC_PATH) PROJECTNAME=$(shell echo $(TARGET_BOARD_PLATFORM_VAR) | tr a-z A-Z) BASEBUILDDIR=$(VMM_BUILD_OUT) WHOLE_ARCHIVE_LIB_LIST+="$(BUILT_LIBSOC_TARGET) $(BUILT_LIB_MOBILEVISOR_SVC_TARGET) $(abspath $(BUILT_MV_CORE_BIN))" PLATFORM=$(MODEM_PLATFORM)
