...
static int create_slcan_socket(void)
{
	...
	getsockopt(s, SOL_SOCKET, SO_RCVBUF, &rcv_buf_size, &optlen);
	default_buf_size = rcv_buf_size;
...
static int suspend(struct canfd_frame *r_frame)
{
	int fd, ret;
	struct canfd_frame t_frame;
	const char *node = POWER_STATE;
	static int last_wakeup_button_sts;
	int rcv_buf_size;

	/* Bit 5 indicates wakeup button */
	if (r_frame->data[0] == e_ias_wakeup_reason_ignition_line){
		ALOGD("wakeup button is not alive; trigerring suspend.\n");
		/* set next shutdown as S3 entry */
		parse_canframe("0000FFFF#08015555555555", &t_frame);
		slcan_send_data(slcan_socket_fd, t_frame);

		/* set rcv_buf to 0. Ingore can frame when suspend */
		rcv_buf_size = 0;
		ret = setsockopt(slcan_socket_fd, SOL_SOCKET, SO_RCVBUF, &rcv_buf_size, sizeof(int));
		...
		ret = write(fd, "mem", strlen("mem"));
		close(fd);

		if (ret == strlen("mem")){
			/* resume back: send stack ready*/
			parse_canframe("0000FFFF#0A005555555555", &t_frame);
			slcan_send_data(slcan_socket_fd, t_frame);
		} else
			ALOGE("Suspend failed\n");

		/* restore the default recv buf size */
		rcv_buf_size = default_buf_size;
		ret = setsockopt(slcan_socket_fd, SOL_SOCKET, SO_RCVBUF, &rcv_buf_size, sizeof(int));
	}

	last_wakeup_button_sts = r_frame->data[0];
...
static int update_temp_data(struct canfd_frame *frame)
{
	...
	if(sprintf(buf, "%u", frame->data[0] * 1000) > 0)
		store_data2node(BOARD_TEMP, buf, 6);
	if(sprintf(buf, "%u", frame->data[2] * 1000) > 0)
		store_data2node(AMBIENT_TEMP, buf, 6);
...
void *slcan_heatbeat_thread(void *x)
{
	struct canfd_frame t_frame;
	...
	while (1) {
		if(recovery_mode)
			parse_canframe("0000FFFF#01025555555555", &t_frame);
		else
...
int main(int argc, char *argv[])
{
	...
	if ((argc > 1) && argv[1] && !strcmp(argv[1], "--recovery"))
		recovery_mode = true;
	...
	while(1) {
		...
		if (r_frame.can_id == (CANID_IOC | CAN_EFF_FLAG)) {
			...
			switch (r_sel)
			{
				case e_ias_slcan_dummy_ctrl_wakeup_reasons:
					{
						suspend(&r_frame);
					}
					break;
