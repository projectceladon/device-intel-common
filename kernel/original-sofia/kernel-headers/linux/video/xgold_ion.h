...
/*
The following enum defines the heaps available on the system.
Note: they are mapped to same heap types that were used before
introducing heap ids, for backwards compatibility.
If new types will be introduced, the two enums will diverge.
See enum in drivers/staging/android/uapi/ion.h
*/
enum {
	XGOLD_ION_HEAP_ID_SYSTEM,
	XGOLD_ION_HEAP_ID_SYSTEM_CONTIG, /* unused */
	XGOLD_ION_HEAP_ID_CARVEOUT,
	XGOLD_ION_HEAP_ID_CHUNK, /* unused */
	XGOLD_ION_HEAP_ID_CMA,
	XGOLD_ION_HEAP_ID_VIDEO, /* = TYPE_CUSTOM currently */
